...
def save_file(dirname):
    ...
    文件内容格式:  类别\t内容
    """
    f_train = open('mydata/cnews.train.txt', 'w', encoding='utf-8')
    f_test = open('mydata/cnews.test.txt', 'w', encoding='utf-8')
    f_val = open('mydata/cnews.val.txt', 'w', encoding='utf-8')
    count = 0
    i,j,k = 1,1,1
    temp = ''
    flag = 0
    for category in os.listdir(dirname):   # 分类目录
        ...
        if flag and (temp[0:2] != category[0:2]):
            i, j, k = 1, 1, 1
        temp = category
        files = os.listdir('total') #此地址为没有做任何数据预处理的数据地址。
        t = count % 5
        filename = os.path.join(dirname, category)
        content = _read_file(filename)
        if category[0:2] == '民政':
            category = category[0:2]
        elif category[0:2] == '政法':
            category = category[0:2]
        elif category[0:9] == '医疗健康与计划生育':
...
if __name__ == '__main__':
    ...
    print(len(open('mydata/cnews.train.txt', 'r', encoding='utf-8').readlines()))
    print(len(open('mydata/cnews.test.txt', 'r', encoding='utf-8').readlines()))
    print(len(open('mydata/cnews.val.txt', 'r', encoding='utf-8').readlines()))
